...
`;customElements.define("view-home",class extends J{static get properties(){return{}}constructor(){super()}goPage(e){e.preventDefault();let t=e.target.getAttribute("name");ne.dispatch(de(t))}static get styles(){return[ce,o`
                :host {
                    position: absolute;
                    display: block;
                    margin: 0;
...
`;class pe extends(me(ne)(J)){static get properties(){return{siteTree:{type:Object},renderValue:{type:String}}}constructor(){super(),this.siteTree={}}stateChanged(e){this.siteTree=e.siteTree,this.renderValue=e.renderValue}idPage(){var e=[];if("project1"==this.renderValue)return[3,3];if("project1Lv"==this.renderValue)return[3,3];for(var t=0;t<this.siteTree.length;t++)if(this.siteTree[t].name==this.renderValue)return e.push(t-1,t+1),e}downPage(e){e.preventDefault();var t=this.siteTree.length;this.idPage()[0]<0?ne.dispatch(de(this.siteTree[t].name)):ne.dispatch(de(this.siteTree[this.idPage()[0]].name))}upPage(e){e.preventDefault();var t=this.siteTree.length;this.idPage()[1]>=t?ne.dispatch(de(this.siteTree[0].name)):ne.dispatch(de(this.siteTree[this.idPage()[1]].name))}goPage(e){if(e.preventDefault(),"project1"==this.renderValue)ne.dispatch(de("projects"));else if("project1Lv"==this.renderValue)ne.dispatch(de("project1"));else{let t=e.target.getAttribute("name");ne.dispatch(de(t))}}titlePagei(){if("project1"==this.renderValue)return"DBAE";for(var e=0;e<this.siteTree.length;e++)if(this.siteTree[e].name==this.renderValue)return this.siteTree[e].title}static get styles(){return[le,o`
                :host {
                    position: relative;
                    display: block;
                    margin: 0;
...
/* Responsive Web Design */
    ...
    @media(min-width: 720px) {
        ...
        .col-11 {width: 91.66%;}
        .col-12 {width: 100%;}
`;class we extends(me(ne)(J)){static get properties(){return{siteTree:{type:Object},renderValue:{type:String},pjPs:{type:Number}}}constructor(){super(),this.siteTree={},this.pjPs=0}stateChanged(e){this.siteTree=e.siteTree,this.renderValue=e.renderValue}goPage(e){e.preventDefault();let t=e.target.getAttribute("name");ne.dispatch(de(t))}static get styles(){return[be,o`
                :host {
                    position: relative;
                    display: block;
                    margin: 0;
...
`;class Ee extends(me(ne)(J)){static get properties(){return{renderValue:{type:String},mSelected:{type:String},nSelected:{type:String},opened:{type:Boolean}}}constructor(){super(),this.mSelected="Inicio",this.nSelected="home"}stateChanged(e){this.renderValue=e.renderValue,console.log(e.renderValue,e.levelValue,e.areaValue)}goPage(e){e.preventDefault();let t=e.target.getAttribute("name");ne.dispatch(de(t))}mgMenu(e){e.preventDefault(),1==this.opened?this.opened=!1:this.opened=!0}mgMenuClose(e){e.preventDefault(),1==this.opened&&(this.opened=!1)}static get styles(){return[ke,o`
                :host {
                    display: block;
                }
            `]}render(){return T`
